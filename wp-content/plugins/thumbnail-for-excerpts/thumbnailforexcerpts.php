...
$optionsTfE=array(
	...
	array(	"name" => "Length",
			"desc" => '<strong>Height:</strong> By default 85, but you can choose any text length.',
			"id" => "tfe_length",
			"std" => "85",
			"type" => "text"
		),
	...
	array(	"name" => "Apply on home",
			...
			"std" => "no",
			"options" => array('yes','no'),
			"type" => "null" //removed "select" so this wouldn't show up in the setting control panel
		),
...
function tfe_get_image($id,$content,$align,$width,$height,$default,$regenerate,$default_src) {
	...
	foreach($imgtype as $type){
		...
		$src = str_replace(".".$type,"-".get_option("thumbnail_size_w")."x".get_option("thumbnail_size_h").".".$type,$src); // Changed to use full size image.
	}
	$noimg=false;
...
function tfe_put_image($excerpt){
	...
	$width=(get_settings('tfe_width')==''?'auto':get_settings('tfe_width'));
	$height=(get_settings('tfe_height')==''?'auto':get_settings('tfe_height'));
	//$length=get_settings('tfe_length');
...
function new_excerpt_length($length) {
	return get_settings('tfe_length');
}
